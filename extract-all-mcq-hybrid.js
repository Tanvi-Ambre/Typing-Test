#!/usr/bin/env node
/**
 * Extract all MCQ batches using hybrid OCR approach
 * Then apply Marathi corrections
 */
...
async function extractPDFHybrid(pdfPath) {
    return new Promise((resolve, reject) => {
        const python = spawn('python3', [
            path.join(__dirname, 'extract-mcq-hybrid.py'),
            pdfPath
        ]);
...
async function extractAllBatches() {
    ...
    for (const file of files) {
        ...
        try {
            console.log(`📝 Extracting ${batchName} with OCR...`);
            const questions = await extractPDFHybrid(pdfPath);
...
function generateMCQDataFile(allBatches) {
    ...
    let content = `// MCQ Questions - Extracted with Hybrid OCR + Marathi corrections
// Generated: ${timestamp}
...
async function main() {
    console.log('🚀 MCQ Batch Extraction with Hybrid OCR + Marathi Correction\n');
