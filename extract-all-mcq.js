// Extract MCQ questions from all BATCH PDFs
// This script uses the sample-questions.json files for each batch
...
// Create question sets for all batches
const allSets = [
    {
    ...
    // Add more batches here as you extract them
];

// Save to JSON
fs.writeFileSync('extracted-mcq.json', JSON.stringify(allSets, null, 2));
console.log(`✅ Fixed Marathi text in ${fixedQuestions.length} questions`);
console.log(`📝 Saved ${allSets.length} batch(es) to: extracted-mcq.json`);
...
console.log(`\n🎉 Done! ${allSets.length} batch(es) with ${allSets.reduce((sum, set) => sum + set.questions.length, 0)} total questions`);
console.log('\nInclude mcq-data.js in your HTML before mcq-script.js');
